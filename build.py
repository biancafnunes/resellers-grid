...
try:
    with open(r'C:\Users\daviaraujo\resellers-grid\daily_lp.csv') as f:
        for row in csv.DictReader(f):
            if row.get('data','').startswith('2026'):
                lp_daily.append({'data':row['data'],'visitas':int(row['visitas'] or 0)})
except FileNotFoundError:
    pass
lp_json = json.dumps(lp_daily)
...
/* ── Grid Bot Share Panel ── */
#gb-panel{position:fixed;top:0;right:0;height:100vh;width:380px;background:#fff;box-shadow:-6px 0 32px rgba(0,0,0,.18);z-index:9999;display:flex;flex-direction:column;transform:translateX(110%);transition:transform .28s cubic-bezier(.4,0,.2,1)}
#gb-panel.open{transform:translateX(0)}
#gb-overlay{display:none;position:fixed;inset:0;z-index:9998;background:transparent}
.gb-action{font-size:20px;cursor:pointer;padding:4px;border-radius:6px;transition:background .12s;line-height:1}
.gb-action:hover{background:#f0f0f0}
.gb-tab{padding:10px 14px;font-size:13px;cursor:pointer;border-bottom:2px solid transparent;color:#888;white-space:nowrap}
.gb-tab.active{color:#1A1F6B;border-bottom-color:#1A1F6B;font-weight:700}
.gb-person{display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f5f5f5}
.gb-avatar{width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:17px;flex-shrink:0;background:#f0f0f0}
.gb-role{font-size:11px;border:1px solid #ddd;border-radius:12px;padding:2px 10px;color:#555;cursor:pointer;margin-left:auto;white-space:nowrap}
.gb-role:hover{background:#f5f5f5}
.gb-share-input{flex:1;border:1px solid #d0d0d0;border-radius:6px;padding:8px 12px;font-size:13px;outline:none;transition:border .15s}
.gb-share-input:focus{border-color:#1A1F6B}
.gb-share-btn{background:#1A1F6B;color:#fff;border:none;border-radius:6px;padding:8px 16px;font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:5px;white-space:nowrap}
.gb-share-btn:hover{background:#22287a}
</style>
</head>
<body>
...
<div style="background:#FFE600;padding:14px 32px;display:flex;align-items:center;justify-content:space-between">
  ...
  <button id="btn-share" onclick="openShareModal()" style="display:inline-flex;align-items:center;gap:8px;background:#1A1F6B;color:#FFE600;border:none;border-radius:20px;padding:8px 20px;font-size:12px;font-weight:700;cursor:pointer;letter-spacing:.05em;text-transform:uppercase">
    &#128279; Compartilhar
  </button>

  <!-- Grid Bot Share Panel -->
  <div id="gb-overlay" onclick="gbClose()"></div>
  <div id="gb-panel">
    <!-- Header -->
    <div style="padding:18px 18px 0;flex-shrink:0">
      <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:12px">
        ...
        <div style="flex:1;min-width:0">
          <div style="font-weight:700;font-size:14px;color:#1a1a2e;margin-bottom:6px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">Reseller 2026</div>
          <div style="display:flex;gap:5px;flex-wrap:wrap">
            <span style="border:1px solid #ddd;border-radius:4px;padding:2px 8px;font-size:11px;color:#555">v1 (latest)</span>
            <span style="border:1px solid #ddd;border-radius:4px;padding:2px 8px;font-size:11px;color:#555">Viewer</span>
            <span style="background:#1a7f37;color:#fff;border-radius:4px;padding:2px 10px;font-size:11px;font-weight:600;cursor:pointer">Public &#9660;</span>
          </div>
        </div>
        <span onclick="gbClose()" style="cursor:pointer;font-size:18px;color:#999;flex-shrink:0;line-height:1;padding:2px">&times;</span>
      </div>
      <!-- Action icons -->
      <div style="display:flex;gap:4px;padding:10px 0;border-bottom:1px solid #eee">
        <span class="gb-action" title="Copy link" onclick="gbCopyLink()">&#128279;</span>
        <span class="gb-action" title="Download" onclick="gbDownload()">&#8659;</span>
        <span class="gb-action" title="Fullscreen" onclick="window.open(GRID_URL,'_blank')" style="margin-left:auto">&#8599;</span>
      </div>
      <!-- Share input -->
      <div style="padding:14px 0 10px">
        <div style="font-size:11px;font-weight:700;color:#888;letter-spacing:.06em;text-transform:uppercase;margin-bottom:8px">Share &#9432;</div>
        <div style="display:flex;gap:8px;margin-bottom:8px">
          <input id="gb-input" class="gb-share-input" type="text" placeholder="Search by name or email..." onkeydown="if(event.key==='Enter')gbShare()"/>
          <button class="gb-share-btn" onclick="gbShare()">&#8776; Share</button>
        </div>
        <button style="border:1px solid #ddd;border-radius:6px;padding:6px 12px;font-size:12px;background:#fff;cursor:pointer;color:#555">&#128101; Groups &#9660;</button>
      </div>
    </div>
    <!-- Tabs -->
    <div style="display:flex;border-bottom:1px solid #eee;padding:0 18px;flex-shrink:0">
      <div class="gb-tab active" id="gbtab-details" onclick="gbTab('details')">Details</div>
      <div class="gb-tab" id="gbtab-activity" onclick="gbTab('activity')">Activity <span style="background:#eee;border-radius:10px;padding:1px 6px;font-size:10px" id="gb-act-count">0</span></div>
      <div class="gb-tab" id="gbtab-comments" onclick="gbTab('comments')">Comments <span style="background:#eee;border-radius:10px;padding:1px 6px;font-size:10px">0</span></div>
    </div>
    <!-- Content -->
    <div id="gb-content" style="flex:1;overflow-y:auto;padding:16px 18px"></div>
    <!-- Footer -->
    <div style="padding:10px 18px;border-top:1px solid #eee;font-size:11px;color:#aaa;display:flex;gap:14px;flex-shrink:0">
      <span style="cursor:pointer;color:#1A1F6B;font-weight:600">My Profile</span>
      <span style="cursor:pointer">/commands</span>
      <span style="cursor:pointer">? shortcuts</span>
      <span style="cursor:pointer">H docs</span>
      <span style="margin-left:auto">v3.6.3</span>
    </div>
  </div>
</div>
...
<div class="tabs">
  ...
  <div class="tab" onclick="showTab('chart-devices',this)">&#128230; DEVICES</div>
  ...
  <div class="grid-wrapper">
    <table>
      <thead><tr>
        ...
        <th data-col="media_ped">Média Ped/Res <span title="Pedidos de devices ÷ Resellers com compra" style="cursor:help;opacity:.6">?</span></th>
        <th data-col="ativos">Sellers Ativos <span title="Devices ativados (com transação) no mês" style="cursor:help;opacity:.6">?</span></th>
        ...
        <th data-col="mom_tpv_m0">MoM TPV M0 <span title="Variação % do TPV M0 vs mês anterior (mês atual = MTD)" style="cursor:help;opacity:.6">?</span></th>
        ...
        <th data-col="mom_ativos">MoM Sellers Ativos <span title="Variação % de devices ativos vs mês anterior" style="cursor:help;opacity:.6">?</span></th>
      </tr></thead>
...
<div class="pane" id="pane-chart-devices">
  ...
  <div style="margin:32px 0 12px"></div>
  <div style="overflow-x:auto;margin-bottom:24px">
    <table style="min-width:500px">
...
<div class="pane" id="pane-diarizado">
  ...
  <!-- Funil diario -->
  <div style="font-size:16px;font-weight:900;color:#1A1F6B;text-transform:uppercase;letter-spacing:.06em;margin-bottom:14px;border-left:5px solid #FFE600;padding-left:14px">Funil Diário</div>
  <div style="background:#fff;border-radius:12px;padding:14px 18px;margin-bottom:14px;box-shadow:0 2px 8px rgba(0,0,0,.06)">
    <div style="font-size:10px;color:#999;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px">Mês</div>
    <div id="mes-filter-funil-diar" style="display:flex;gap:6px;flex-wrap:wrap"></div>
  </div>
...
<!-- ── TAB SUBIDAS / QUEDAS DE NÍVEL ── -->
<div class="pane" id="pane-niveis">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:12px">
    <div style="font-size:18px;font-weight:900;color:#1A1F6B;text-transform:uppercase;letter-spacing:.08em;border-left:5px solid #FFE600;padding-left:14px" id="titulo-niveis">Subidas de Nível — 2026</div>
    <div style="display:flex;gap:0;border-radius:8px;overflow:hidden;border:2px solid #1A1F6B">
      <button id="btn-subidas" onclick="setVistaNivel('subidas')" style="padding:7px 20px;font-size:12px;font-weight:700;background:#1A1F6B;color:#FFE600;border:none;cursor:pointer;letter-spacing:.04em">&#11014; SUBIDAS</button>
      <button id="btn-quedas" onclick="setVistaNivel('quedas')" style="padding:7px 20px;font-size:12px;font-weight:700;background:#fff;color:#1A1F6B;border:none;cursor:pointer;letter-spacing:.04em">&#11015; QUEDAS</button>
    </div>
  </div>
  ...
  <div id="view-subidas">
    ...
    <div class="chart-wrap">
      ...
      <div class="chart-sub">Quantidade de resellers que subiram de nível — Jan–Mai/26</div>
      <canvas id="chartNiveis"></canvas>
    </div>
  </div>

  <!-- QUEDAS -->
  <div id="view-quedas" style="display:none">
    <div class="cards" id="cards-quedas"></div>
    <div class="controls" style="margin-bottom:16px">
      <div id="mes-filter-quedas" style="display:flex;gap:6px;flex-wrap:wrap"></div>
      <div id="trans-filter-quedas" style="display:flex;gap:6px;flex-wrap:wrap"></div>
      <input class="search-box" type="text" placeholder="Buscar reseller..." id="search-quedas"/>
    </div>
    <div class="grid-wrapper" style="margin-bottom:24px">
      <table>
        <thead><tr>
          ...
          <th data-col-n="reseller">Reseller ID</th>
          <th data-col-n="transicao">Transição</th>
          <th data-col-n="nivel_anterior">Nível Anterior</th>
          <th data-col-n="nivel_atual">Nível Atual</th>
        </tr></thead>
    ...
    <div class="chart-wrap">
      <div class="chart-title">Quedas por Transição e Mês</div>
      <div class="chart-sub">Quantidade de resellers que caíram de nível — Fev–Mai/26</div>
      <canvas id="chartQuedas"></canvas>
    </div>
  </div>
</div>
...
<script>
const RAW = [
  {mes:"Jan/26",nivel:"Aprendiz",        resellers:914,  pedidos:2884, ativos:5875, tpv_m0:5907164,  tpv_m1:null,      tpv_total:5907164,   pedidos_pro:2409, pedidos_smart:475, ativos_pro:5181, ativos_smart:694},
  {mes:"Jan/26",nivel:"Especialista",     resellers:113,  pedidos:1397, ativos:124, tpv_m0:3043182,  tpv_m1:null,      tpv_total:3043182,   pedidos_pro:1231, pedidos_smart:166, ativos_pro:109, ativos_smart:15},
  ...
  {mes:"Fev/26",nivel:"Aprendiz",        resellers:983,  pedidos:2834, ativos:5848, tpv_m0:5641087,  tpv_m1:8592273,  tpv_total:14233360,  pedidos_pro:2418, pedidos_smart:416, ativos_pro:4905, ativos_smart:943},
  {mes:"Fev/26",nivel:"Especialista",     resellers:117,  pedidos:1338, ativos:99,   tpv_m0:3643734,  tpv_m1:4613736,  tpv_total:8257469,   pedidos_pro:1199, pedidos_smart:139, ativos_pro:87,   ativos_smart:12},
  {mes:"Fev/26",nivel:"Empreendedor",     resellers:52,   pedidos:1246, ativos:163,  tpv_m0:2681090,  tpv_m1:4726280,  tpv_total:7407369,   pedidos_pro:1055, pedidos_smart:191, ativos_pro:134,  ativos_smart:29},
  {mes:"Fev/26",nivel:"Top Empreendedor", resellers:38,   pedidos:2111, ativos:510,  tpv_m0:6183877,  tpv_m1:12285399, tpv_total:18469277,  pedidos_pro:1350, pedidos_smart:761, ativos_pro:345,  ativos_smart:165},
  {mes:"Mar/26",nivel:"Aprendiz",        resellers:922,  pedidos:2467, ativos:6315, tpv_m0:6655918,  tpv_m1:7923691,  tpv_total:22599539,  pedidos_pro:1958, pedidos_smart:509, ativos_pro:5140, ativos_smart:1175},
  ...
  {mes:"Abr/26",nivel:"Aprendiz",        resellers:774,  pedidos:1656, ativos:5502, tpv_m0:6296783,  tpv_m1:8248703,  tpv_total:29353889,  pedidos_pro:1218, pedidos_smart:438, ativos_pro:4357, ativos_smart:1145},
  ...
  {mes:"Mai/26",nivel:"Aprendiz",        resellers:1033, pedidos:2372, ativos:5424, tpv_m0:5578635,  tpv_m1:9021238,  tpv_total:36236971,  pedidos_pro:2158, pedidos_smart:214, ativos_pro:4339, ativos_smart:1085},
  ...
  {mes:"Jun/26",nivel:"Aprendiz",        resellers:358,  pedidos:673, ativos:1084, tpv_m0:1304117,  tpv_m1:2963303,  tpv_total:15465678,  pedidos_pro:606,  pedidos_smart:67, ativos_pro:976,   ativos_smart:108},
  {mes:"Jun/26",nivel:"Especialista",        resellers:63,  pedidos:462, ativos:333, tpv_m0:576818,  tpv_m1:1831415,  tpv_total:8076314,  pedidos_pro:445,  pedidos_smart:17, ativos_pro:321,   ativos_smart:12},
  {mes:"Jun/26",nivel:"Empreendedor",        resellers:36,  pedidos:514, ativos:300, tpv_m0:681701,  tpv_m1:2290836,  tpv_total:10626272,  pedidos_pro:437,  pedidos_smart:77, ativos_pro:255,   ativos_smart:45},
  {mes:"Jun/26",nivel:"Top Empreendedor",        resellers:31,  pedidos:634, ativos:622, tpv_m0:970981,  tpv_m1:3809971,  tpv_total:20870712,  pedidos_pro:546,  pedidos_smart:88, ativos_pro:536,   ativos_smart:86},
];
...
function renderTabela(){
  ...
  data.forEach((r,i)=>{
    ...
    const tpvMedM0=r.ativos?r.tpv_m0/r.ativos:0;
    const prevAtivM1=getMoM(r.nivel,r.mes,'ativos');
    const tpvMedM1=prevAtivM1&&r.tpv_m1!=null?r.tpv_m1/prevAtivM1:null;
    ...
    html+=`<tr style="background:${mesBg}">
      <td style="border-left:3px solid ${mesBg==='#ffffff'?'#1A1F6B':'#888'}">${isFirst?`<span class="mes-badge">${r.mes}</span>`:''}</td>
      ...
      <td style="text-align:right;font-weight:600;color:#1A1F6B">${r.resellers?( r.pedidos/r.resellers).toFixed(1):'-'}</td>
      ...
      <td style="text-align:center">${fmt(r.tpv_m0)}${momBadge(r.tpv_m0,prevTpvM0,isMTD)}</td>
    ...
    if(isLast&&!activeMes&&!sortCol){
      ...
      const prevMesAtivos=(()=>{const idx=MES_ORDER.indexOf(r.mes);if(idx<=0)return 0;const pm=MES_ORDER[idx-1];return RAW.filter(x=>x.mes===pm&&(!activeNivel||x.nivel===activeNivel)).reduce((s,x)=>s+x.ativos,0);})();
      const totMedM1=prevMesAtivos&&t.haM1?t.tpv_m1/prevMesAtivos:null;
      const totBg=mesBg==='#ffffff'?'#e8e8e8':'#dcdcdc';
      const totMediaPed=t.resellers?(t.pedidos/t.resellers).toFixed(1):'-';
      html+=`<tr class="group-total" style="background:${totBg}"><td><span class="mes-badge">${r.mes}</span></td><td style="font-size:11px">TOTAL</td>
        <td>${fmtN(t.resellers)}</td><td>${fmtN(t.pedidos)}</td><td style="text-align:right;font-weight:700;color:#1A1F6B">${totMediaPed}</td><td>${fmtN(t.ativos)}</td>
        <td>${fmt(t.tpv_m0)}</td><td>${t.haM1?fmt(t.tpv_m1):fmt(null)}</td>
  ...
  document.getElementById('tbody').innerHTML=html||'<tr><td colspan="14" style="text-align:center;color:#ccc;padding:32px">Sem resultados</td></tr>';
}
...
function renderCardTabela(){
  ...
  document.getElementById('cards-tabela').innerHTML=`
    ...
    <div class="card"><div class="card-label">Sellers Ativos</div><div class="card-value">${fmtN(ta)}</div><div class="card-sub">Jan-Abr/26</div></div>
...
function exportCSV(){
  const data=getData();
  const header=['Mes','Nivel','Resellers','Dev.Pedidos','Sellers Ativos','TPV_M0','TPV_M1','TPV_Total','Ticket_Medio'];
...
function renderChartDevices(){
  ...
  const shortDD=d=>{
    ...
    return `<span style="font-size:9px;opacity:.75;display:block;${isWeekend?'color:#FFE600':''}">${diaSem}</span>${d.slice(8)}/${d.slice(5,7)}`;
  };
  ...
  if(devDias.length){
    ...
    if(window._devDailyNivel==='Todos'){
      ...
      const tdTot='padding:10px 12px;text-align:right;font-size:12px;font-weight:900;background:#1A1F6B;color:#FFE600;border-bottom:1px solid #1A1F6B';
      devDailyHtml+=`<tr style="background:#1A1F6B;font-weight:900;color:#FFE600">
        <td style="padding:10px 14px;font-size:12px;border-bottom:1px solid #1A1F6B">TOTAL</td>
        ${totVals.map(v=>`<td style="${tdTot}">${fmtN(v)}</td>`).join('')}
        <td style="${tdTot};background:#FFE600;color:#1A1F6B">${fmtN(grand)}</td>
        <td style="${tdTot}">${mediaTot}</td>
      </tr>`;
    }
  }
...
function renderDiarizado(){
  ...
  // Funil diário — dias como colunas
  ...
  document.getElementById('thead-funil-row').innerHTML=
    ...
    funilDias.map(d=>`<th style="${thFS}">${shortFD(d)}</th>`).join('')+
    `<th style="${thFS};background:#FFE600;color:#1A1F6B;font-weight:900">TOTAL</th>`;

  function funilRow(label,nums,fmtFn,totOverride){
    const colors=colorScale(nums);
    const tot=nums.reduce((s,v)=>s+v,0);
    const totDisplay=totOverride!==undefined?totOverride:fmtFn(tot);
    return `<tr>
      ...
      ${nums.map((v,i)=>`<td style="padding:10px 12px;text-align:right;font-size:12px;border-bottom:1px solid #f0f0f0;${colors[i]}">${fmtFn(v)}</td>`).join('')}
      <td style="padding:10px 12px;text-align:right;font-size:12px;border-bottom:1px solid #f0f0f0;background:#fffde7;font-weight:800;color:#1A1F6B">${totDisplay}</td>
    </tr>`;
  }
  ...
  document.getElementById('mes-filter-tpv-dia').innerHTML=MES_ORDER.filter(m=>m!=='Mai/26').map(m=>
...
// ── SUBIDAS DE NÍVEL ──
const SUBIDAS_DETAIL = [{"reseller": "497552313", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "225718686", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "586326645", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "624974985", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "1226761249", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "1288797521", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "47221809", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "42318297", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "3146784575", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "383258287", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "3118113625", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "144090312", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "140888320", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "327251168", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "72873975", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "190822347", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "194881563", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "64254937", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "196388646", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "299222705", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "1430493190", "nivel_atual": "Top Empreendedor", "dt": "2026-02-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "75554443", "nivel_atual": "Top Empreendedor", "dt": "2026-02-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "3057984821", "nivel_atual": "Empreendedor", "dt": "2026-02-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "150759987", "nivel_atual": "Empreendedor", "dt": "2026-02-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "296824220", "nivel_atual": "Empreendedor", "dt": "2026-02-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "242763116", "nivel_atual": "Empreendedor", "dt": "2026-02-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "342576897", "nivel_atual": "Empreendedor", "dt": "2026-02-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "1208501777", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "1425305619", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "1482768258", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "1748497382", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "134179496", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "1767434152", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "2155784259", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "2195196707", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "2445794092", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "232802020", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "656772499", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "20970977", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "213550906", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "241222977", "nivel_atual": "Top Empreendedor", "dt": "2026-03-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "1388425394", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "350840506", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "1150985186", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "2087174154", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "149177824", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "402580963", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "1392779078", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "42318297", "nivel_atual": "Top Empreendedor", "dt": "2026-03-01", "transicao": "Especialista -> Top Empreendedor"}, {"reseller": "203850521", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "130855841", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "463286021", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "456562778", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "186371630", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "730679045", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "2191289064", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "246343377", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "164546775", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "1009968348", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "1339892065", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "603986736", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "2619742737", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "1128756788", "nivel_atual": "Top Empreendedor", "dt": "2026-04-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "2087174154", "nivel_atual": "Top Empreendedor", "dt": "2026-04-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "573284473", "nivel_atual": "Top Empreendedor", "dt": "2026-04-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "495437190", "nivel_atual": "Top Empreendedor", "dt": "2026-04-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "2176828104", "nivel_atual": "Top Empreendedor", "dt": "2026-04-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "1615066500", "nivel_atual": "Top Empreendedor", "dt": "2026-04-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "2385034802", "nivel_atual": "Top Empreendedor", "dt": "2026-04-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "241572613", "nivel_atual": "Empreendedor", "dt": "2026-04-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "2445794092", "nivel_atual": "Empreendedor", "dt": "2026-04-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "1532390983", "nivel_atual": "Empreendedor", "dt": "2026-04-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "1748497382", "nivel_atual": "Empreendedor", "dt": "2026-04-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "472166356", "nivel_atual": "Empreendedor", "dt": "2026-04-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "153688180", "nivel_atual": "Empreendedor", "dt": "2026-04-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "1226761249", "nivel_atual": "Empreendedor", "dt": "2026-04-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "468798445", "nivel_atual": "Empreendedor", "dt": "2026-04-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "1318674044", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "140888320", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "652701629", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "266473468", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "652551126", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "530372985", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "47472337", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "489075905", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "70122864", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Aprendiz -> Especialista"}, {"reseller": "187154877", "nivel_atual": "Top Empreendedor", "dt": "2026-05-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "1964066515", "nivel_atual": "Top Empreendedor", "dt": "2026-05-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "438097380", "nivel_atual": "Top Empreendedor", "dt": "2026-05-01", "transicao": "Empreendedor -> Top Empreendedor"}, {"reseller": "1124666881", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "150759987", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "269557672", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "2155784259", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "229880050", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Especialista -> Empreendedor"}, {"reseller": "1095425016", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Especialista -> Empreendedor"}];

const QUEDAS_DETAIL = [{"reseller": "128107890", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "1124666881", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "468798445", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "1608234266", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "2269348938", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "2546726800", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "349576623", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "559838319", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "89625832", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "133221394", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-02-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "150545593", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "496242609", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "139077866", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "182077801", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "142424348", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "340208278", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "146339363", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "147300310", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "328674693", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "167246301", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "311101389", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "309147557", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "205173059", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "530372985", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "7336033", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1303450632", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "453530101", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1262865124", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "427885748", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1127400240", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "5781283", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1078249656", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1004140597", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "777169239", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "114165543", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-02-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "495437190", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-02-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "8467009", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-02-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "203675208", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "81397558", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "324154304", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "404047390", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "387096040", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "1095425016", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "234846212", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "818833450", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "249216814", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-03-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "3146784575", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "70122864", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "528817069", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "568514824", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "96613116", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "697119621", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "797899359", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "47472337", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "420814739", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "140888320", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1085204839", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "259928287", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1834809019", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "2546726800", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "98627661", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1742983151", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1608234266", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "47791019", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1603559239", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "225718686", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "48909778", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "355555035", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "72873975", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "507780494", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "319250547", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "337487749", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "2260799577", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-03-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "187154877", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "218399870", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "225494908", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "1128756788", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "207447678", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-03-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "247131918", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "150759987", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "545579977", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-04-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "1288797521", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "194881563", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "299222705", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "196388646", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "476815869", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1004740857", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "443012670", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1573655102", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "116157946", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1750530345", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "571359051", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "266473468", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "118578835", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-04-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "440374085", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-04-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "1388425394", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "623246996", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "1152023243", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "1935378054", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "350840506", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "1068570330", "nivel_anterior": "Empreendedor", "nivel_atual": "Especialista", "dt": "2026-05-01", "transicao": "Empreendedor -> Especialista"}, {"reseller": "73798340", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "2193060548", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "313851086", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "174934943", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "144090312", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1471223038", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "172116091", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "811153073", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "201283942", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "502170173", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "231537189", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "114376909", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "90397090", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "271326279", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "234846212", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "241201922", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "346001153", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "247131918", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "1208501777", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "605996723", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "93294701", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "41011640", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "536831719", "nivel_anterior": "Especialista", "nivel_atual": "Aprendiz", "dt": "2026-05-01", "transicao": "Especialista -> Aprendiz"}, {"reseller": "387319387", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "534365187", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "1777649767", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "699129892", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "272079697", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "290610770", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Top Empreendedor -> Empreendedor"}, {"reseller": "1652590161", "nivel_anterior": "Top Empreendedor", "nivel_atual": "Empreendedor", "dt": "2026-05-01", "transicao": "Top Empreendedor -> Empreendedor"}];
...
const TRANS_ORDER_Q = ["Especialista -> Aprendiz","Empreendedor -> Especialista","Top Empreendedor -> Empreendedor"];
...
let activeMesQ=null, activeTransQ=null, searchQ='', chartQuedas=null;

function setVistaNivel(vista){
  const isSub = vista==='subidas';
  document.getElementById('view-subidas').style.display = isSub?'block':'none';
  document.getElementById('view-quedas').style.display  = isSub?'none':'block';
  document.getElementById('btn-subidas').style.background = isSub?'#1A1F6B':'#fff';
  document.getElementById('btn-subidas').style.color = isSub?'#FFE600':'#1A1F6B';
  document.getElementById('btn-quedas').style.background  = isSub?'#fff':'#ef4444';
  document.getElementById('btn-quedas').style.color  = isSub?'#1A1F6B':'#fff';
  document.getElementById('titulo-niveis').textContent = isSub?'Subidas de Nível — 2026':'Quedas de Nível — 2026';
  if(!isSub) renderQuedas();
}

function renderQuedas(){
  const MESES_Q = ["2026-02","2026-03","2026-04","2026-05"];
  let d=[...QUEDAS_DETAIL];
  if(activeMesQ) d=d.filter(r=>r.dt.startsWith(activeMesQ));
  if(activeTransQ) d=d.filter(r=>r.transicao===activeTransQ);
  if(searchQ) d=d.filter(r=>r.reseller.includes(searchQ)||r.transicao.toLowerCase().includes(searchQ.toLowerCase()));
  d.sort((a,b)=>b.dt.localeCompare(a.dt));

  const total=QUEDAS_DETAIL.length;
  const totByTrans={};
  TRANS_ORDER_Q.forEach(t=>totByTrans[t]=QUEDAS_DETAIL.filter(r=>r.transicao===t).length);

  document.getElementById('cards-quedas').innerHTML=`
    <div class="card" style="border-left-color:#ef4444"><div class="card-label">Total Quedas</div><div class="card-value" style="color:#ef4444">${total}</div><div class="card-sub">Fev–Mai/26</div></div>
    ${TRANS_ORDER_Q.map(t=>`<div class="card" style="border-left-color:${TRANS_COLOR_Q[t]}">
      <div class="card-label" style="font-size:9px">${t.replace(' -> ',' → ')}</div>
      <div class="card-value" style="color:${TRANS_COLOR_Q[t]}">${totByTrans[t]||0}</div>
      <div class="card-sub">Fev–Mai/26</div></div>`).join('')}`;

  let html='';
  d.forEach(r=>{
    const cor=TRANS_COLOR_Q[r.transicao]||'#ef4444';
    const nc=NIV_COLOR_MAP[r.nivel_atual]||'#999';
    const np=NIV_COLOR_MAP[r.nivel_anterior]||'#999';
    html+=`<tr>
      <td>${MES_LABEL_N[r.dt.slice(0,7)]||r.dt.slice(0,7)}</td>
      ...
      <td><span class="nivel-badge"><span class="nivel-dot" style="background:${np}"></span>${r.nivel_anterior}</span></td>
      <td><span class="nivel-badge"><span class="nivel-dot" style="background:${nc}"></span>${r.nivel_atual}</span></td>
    </tr>`;
  });
  document.getElementById('tbody-quedas').innerHTML=html||'<tr><td colspan="5" style="text-align:center;color:#ccc;padding:32px">Sem resultados</td></tr>';

  document.getElementById('mes-filter-quedas').innerHTML=['Todos',...MESES_Q].map(m=>
    `<button class="filter-btn${(!activeMesQ&&m==='Todos')||activeMesQ===m?' active':''}" onclick="activeMesQ='${m==='Todos'?'':m}';renderQuedas()">${m==='Todos'?'Todos':MES_LABEL_N[m]}</button>`).join('');
  document.getElementById('trans-filter-quedas').innerHTML=['Todos',...TRANS_ORDER_Q].map(t=>
    `<button class="filter-btn${(!activeTransQ&&t==='Todos')||activeTransQ===t?' active':''}" onclick="activeTransQ='${t==='Todos'?'':t}';renderQuedas()">${t==='Todos'?'Todos':TRANS_LABEL_Q[t]}</button>`).join('');
  document.getElementById('search-quedas').addEventListener('input',e=>{searchQ=e.target.value.trim();renderQuedas();});

  if(chartQuedas) chartQuedas.destroy();
  chartQuedas=new Chart(document.getElementById('chartQuedas').getContext('2d'),{
    type:'bar',
    data:{labels:MESES_Q.map(m=>MES_LABEL_N[m]),
      datasets:TRANS_ORDER_Q.map(t=>({label:t.replace(' -> ',' → '),
        data:MESES_Q.map(m=>QUEDAS_DETAIL.filter(r=>r.transicao===t&&r.dt.startsWith(m)).length),
        backgroundColor:TRANS_COLOR_Q[t],borderRadius:4}))},
    options:{responsive:true,plugins:{legend:{position:'top',labels:{font:{size:11},usePointStyle:true}},
...
function renderNiveis(){
  ...
  document.getElementById('cards-niveis').innerHTML=`
    <div class="card"><div class="card-label">Total Subidas</div><div class="card-value">${total}</div><div class="card-sub">Fev–Mai/26</div></div>
...
function renderAprendiz(){
  ...
  document.getElementById('cards-aprendiz').innerHTML=`
    <div class="card"><div class="card-label">Total Aprendiz c/ Compra</div><div class="card-value">${fmtN(totGeral)}</div><div class="card-sub">Jan–Mai/26</div></div>
...
function renderEspecialista(){
  ...
  document.getElementById('cards-especialista').innerHTML=`
    <div class="card" style="border-left-color:#3b82f6"><div class="card-label">Total Especialista c/ Compra</div><div class="card-value" style="color:#3b82f6">${fmtN(totG)}</div><div class="card-sub">Jan–Mai/26</div></div>
...
function renderFunilTab(){
  ...
  document.getElementById('funil-visual').innerHTML =
    ...
    funnelStage(100, '#1A1F6B', '#ffffff', 'Devices Pedidos', fmtN(pedidos),  funilMesTab==='Todos'?'Jan–Mai/26':funilMesTab, `${pctAtiv}% foram ativados ▼`) +
    funnelStage(78,  '#FFE600', '#1A1F6B', 'Sellers Ativos',  fmtN(ativos),  `${pctAtiv}% dos pedidos`, `TPV dos sellers M0 ▼`) +
...
function renderRmkt(){
  ...
  if(RMKT_DATA.length){
    ...
    html1+=`<tr style="background:#1A1F6B">
      <td style="padding:12px 16px;font-size:13px;font-weight:900;color:#FFE600;border-top:2px solid #FFE600">TOTAL</td>
      <td style="${tdT}">${fmtN2(t1.custs_impactados)}</td>
      <td style="${tdT}">${fmtN2(t1.reativados)}</td>
      <td style="${tdT}">${tx1}</td>
  ...
  if(RMKT_REATIV.length){
    ...
    html2+=`<tr style="background:#1A1F6B">
      <td style="padding:12px 16px;font-size:13px;font-weight:900;color:#FFE600;border-top:2px solid #FFE600">TOTAL</td>
      <td style="${tdT2}">${fmtN2(t2.zerados_abril)}</td>
      <td style="${tdT2}">${fmtN2(t2.voltaram_maio)}</td>
      <td style="${tdT2}">${tx2}</td>
...
const GRID_CHANNEL = 'D0ARBNVBAHK';
const GRID_URL     = 'https://davidaraujo-alt.github.io/resellers-grid/';

// ── People with access (persiste em localStorage) ──────────────────────
const AVATARS = ['&#128512;','&#128525;','&#129303;','&#128526;','&#129321;','&#128519;','&#129325;','&#128516;','&#129322;','&#128518;'];
function gbLoadPeople(){
  try{ return JSON.parse(localStorage.getItem('gb_people')||'null'); }catch(e){ return null; }
}
function gbSavePeople(p){ localStorage.setItem('gb_people', JSON.stringify(p)); }
function gbInitPeople(){
  let p = gbLoadPeople();
  if(!p) p = [{name:'daviaraujo',role:'Owner',avatar:'&#128526;'}];
  return p;
}

// ── Painel ──────────────────────────────────────────────────────────────
let gbActiveTab = 'details';
let gbActivity  = [];

function openShareModal(){ gbOpen(); }   // compatibilidade com btn antigo

function gbOpen(){
  document.getElementById('gb-panel').classList.add('open');
  document.getElementById('gb-overlay').style.display = 'block';
  gbRender();
  setTimeout(()=>document.getElementById('gb-input').focus(), 280);
}
function gbClose(){
  document.getElementById('gb-panel').classList.remove('open');
...
function gbTab(tab){
  ...
  document.getElementById('gbtab-'+tab).classList.add('active');
  gbRender();
}
function gbRender(){
  const el = document.getElementById('gb-content');
  if(gbActiveTab==='details') el.innerHTML = gbRenderDetails();
  if(gbActiveTab==='activity') el.innerHTML = gbRenderActivity();
  if(gbActiveTab==='comments') el.innerHTML = '<div style="text-align:center;color:#ccc;padding:40px 0;font-size:13px">Sem comentarios</div>';
  document.getElementById('gb-act-count').textContent = gbActivity.length;
}
function gbRenderDetails(){
  const people = gbInitPeople();
  const rows = people.map((p,i)=>`
    <div class="gb-person">
      <div class="gb-avatar">${p.avatar||AVATARS[i%AVATARS.length]}</div>
      <span style="font-size:13px;font-weight:500;color:#1a1a2e">${p.name}</span>
      <span class="gb-role">${p.role} &#9660;</span>
    </div>`).join('');
  return `
    <div style="font-size:12px;font-weight:700;color:#555;margin-bottom:10px">&#9660; People with access</div>
    ${rows}
    <div style="font-size:12px;font-weight:700;color:#555;margin:22px 0 10px">&#9660; File details</div>
    <div style="font-size:12px;color:#888">
      <div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid #f5f5f5"><span>Type</span><span style="color:#333">HTML Dashboard</span></div>
      <div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid #f5f5f5"><span>Atualizado</span><span style="color:#333">05/05/2026</span></div>
      <div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid #f5f5f5"><span>Fonte</span><span style="color:#1A1F6B">GitHub Pages</span></div>
      <div style="display:flex;justify-content:space-between;padding:7px 0"><span>Link</span><a href="${GRID_URL}" target="_blank" style="color:#1A1F6B;font-size:11px;word-break:break-all">resellers-grid</a></div>
    </div>`;
}
function gbRenderActivity(){
  if(!gbActivity.length) return '<div style="text-align:center;color:#ccc;padding:40px 0;font-size:13px">Sem atividade recente</div>';
  return gbActivity.slice().reverse().map(a=>`
    <div style="display:flex;gap:8px;padding:10px 0;border-bottom:1px solid #f5f5f5">
      <div class="gb-avatar" style="font-size:14px">${a.avatar}</div>
      <div>
        <div style="font-size:12px;color:#1a1a2e"><b>${a.name}</b> ${a.action}</div>
...
function gbShare(){
  const raw = document.getElementById('gb-input').value.trim().replace(/^@/,'');
  if(!raw){ document.getElementById('gb-input').focus(); return; }

  const names = raw.split(/[,;]+/).map(s=>s.trim()).filter(Boolean);
  const people = gbInitPeople();
  const ai = AVATARS[people.length % AVATARS.length];

  names.forEach(name=>{
    if(!people.find(p=>p.name===name)){
      people.push({name, role:'Viewer', avatar: ai});
      gbActivity.push({name, avatar: ai, action:`ganhou acesso como Viewer`, time: new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})});
    }
  });
  gbSavePeople(people);
  document.getElementById('gb-input').value = '';

  // Copia mensagem e abre Grid Bot
  const msg = `Compartilhando Reseller 2026 com: ${names.join(', ')}\n${GRID_URL}`;
  navigator.clipboard.writeText(msg).then(()=>{
    window.open('https://slack.com/app_redirect?channel='+GRID_CHANNEL,'_blank');
  });
  gbRender();
}

function gbCopyLink(){
  navigator.clipboard.writeText(GRID_URL);
...
function gbDownload(){
  const a = document.createElement('a');
  a.href = GRID_URL; a.download = 'reseller_2026.html'; a.click();
}
