...
function renderDiarizado(){
  ...
  const primeiraEverDia = funilDias.map(d=>{const r=FUNIL_APP.find(x=>x.data===d);return r?r.primeira_compra_ever:0;});

  const totLP       = lpDia.reduce((s,v)=>s+v,0);
  const totCadF     = cadDia.reduce((s,v)=>s+v,0);
  const totPrimApp  = primeiraAppDia.reduce((s,v)=>s+v,0);
  const totPrimEver = primeiraEverDia.reduce((s,v)=>s+v,0);
  ...
  document.getElementById('tbody-funil').innerHTML=funilDias.length?[
    ...
    funilRow('1ª Compra — cohort do mês',      primeiraAppDia,  v=>fmtN(v), `${fmtN(totPrimApp)} <span style="font-size:11px;color:#1A1F6B;font-weight:900">(${taxaApp})</span>`),
    funilRow('1ª Compra — qualquer cadastro',  primeiraEverDia, v=>fmtN(v), fmtN(totPrimEver)),
